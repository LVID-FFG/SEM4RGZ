...
const DoorDetail = () => {
	...
	if (!sealResult.error) {
	    ...
	    if (sealResult.data && sealResult.data.status === 'broken') {
	        const now = new Date();
	        const brokenTime = new Date(sealResult.data.brokenAt);
	        const diffMinutes = (now - brokenTime) / 1000 / 60;
	        if (diffMinutes > 2) {
	            setSealInfo({ ...sealResult.data, hideBroken: true });
	        }
	    }
	} else if (sealResult.status !== 404) {
	    // Если ошибка не 404 (например, 401/403), то показываем сообщение
	    setError('Не удалось загрузить информацию о пломбе');
	}
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
                    ...
                    {sealInfo && sealInfo.status === 'installed' && (
                        <>
                            ...
                            <p><strong>Дата последнего изменения:</strong> {new Date(sealInfo.changedAt).toLocaleString()}</p>
                            {sealInfo.installedByName && (
                                <p><strong>Установил:</strong> {sealInfo.installedByName}</p>
                            )}
                        </>
                    )}
                    {sealInfo && sealInfo.status === 'broken' && !sealInfo.hideBroken && (
                        <p style={{ color: 'var(--danger-color)', fontWeight: 'bold' }}>
                            Пломба взломана! Время взлома: {new Date(sealInfo.brokenAt).toLocaleString()}
                        </p>
                    )}
                    {sealInfo && sealInfo.status === 'disabled' && (
                        <p style={{ color: 'var(--warning-color)', fontWeight: 'bold' }}>
                            Пломба отключена! Время отключения: {new Date(sealInfo.disabledAt).toLocaleString()}
                        </p>
                    )}
                    {sealInfo && sealInfo.status === 'removed' && (
                        <p style={{ color: 'var(--gray-500)', fontWeight: 'bold' }}>
                            Пломба снята! Время снятия: {new Date(sealInfo.removedAt).toLocaleString()}
                        </p>
                    )}
