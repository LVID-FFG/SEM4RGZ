...
const Login = () => {
    ...
    // Очистка ошибок из location.state
    useEffect(() => {
    ...
    const handleLogin = async (e) => {
        ...
        if (result.success) {
            if (result.requires2FA) {
                ...
                // Проверяем, что from не равен '/login' и не пустой
                if (from === '/login' || from === '' || from === '/') {
                    // Если пользователь пытается попасть на /login, отправляем на главную
    ...
    const handle2FAVerify = async (e) => {
        ...
        const result = await auth.verify2FA(userId, twoFactorCode);
    ...
    const handleResetPassword = async (e) => {
        ...
        setIsLoading(true);
        setError('');
        const result = await auth.resetPassword(username, resetCode, newPassword);
        if (result.success) {
            ...
            <div style={{ 
                ...
                <div style={{ 
                    ...
                    <div style={{ marginTop: '1.5rem', textAlign: 'center' }}>
                        <button onClick={handleResetBack} className="btn btn-secondary" style={{ width: '100%' }} disabled={isLoading}>
                            Вернуться ко входу
                        </button>
                    </div>
                </div>
